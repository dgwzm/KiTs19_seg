...
class Visualiser():
    def __init__(self, opt, save_dir, filename='loss_log.txt'):
        ...
        if self.display_id > 0:
            import visdom
            #self.vis = visdom.Visdom(env="tile")
            self.vis = visdom.Visdom(server=opt.server,port=opt.display_port)
            print("get server port")
    ...
    #(epoch: 4, split: train) Seg_Loss: 0.980
    #(epoch: 4, split: validation) Seg_Loss: 0.983 Overall_Acc: 0.578 Mean_IOU: 0.292 Class_0: 0.721 Class_1: 0.017
    #(epoch: 4, split: test) Seg_Loss: 0.983 Overall_Acc: 0.578 Mean_IOU: 0.292 Class_0: 0.721 Class_1: 0.017

    def plot_line(self, x, y, key, split_name):
                    # 0,0.983,Seg_Loss,train
        if key not in self.error_plots:
            ...
            #self.error_wins[key] = self.display_id * 3 + len(self.error_wins)
            #self.error_plots[key] = \

            self.vis.line(X=np.array([x]),Y=np.array([y]),win=self.error_wins[key],update='append' ,opts=dict(legend=[split_name]))
...
class ViShow():
    def __init__(self,opts,models,x,filename='loss_log.txt'):
        
        self.writer=SummaryWriter(opts.pth+"/"+opts.summary_name)
        #self.writer.add_graph(models.net,(x))

        self.log_name = os.path.join(models.save_dir, filename)
        with open(self.log_name, "a+") as log_file:
            now = time.strftime("%c")
            log_file.write('================ Training Loss (%s) ================\n' % now)
    ...
    def plot_current_errors(self, epoch, errors, split_name):
        for k, v in errors.items():
            self.writer.add_scalar(split_name+"_"+k, v,epoch)
