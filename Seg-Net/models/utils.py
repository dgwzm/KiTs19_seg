...
def get_optimizer(option, params):
    #opt_alg = 'adam' if not hasattr(option, 'optim') else option.optim
    opt_alg = option.optim
    if opt_alg == 'sgd':
...
def segmentation_stats(pred_seg, target):
    ...
    #print("segm pre",pred_lbls.shape,"target",target.shape)
