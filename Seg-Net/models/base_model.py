...
class BaseModel():
    ...
    def save(self, label,loss):
        pass

    # helper saving function that can be used by subclasses
    def save_network(self, network, network_label, epoch_label, gpu_ids,loss_itm):
        print('Saving the model {0} at the end of epoch {1}'.format(network_label, epoch_label))
        #save_filename = "{0:03d}_net_{:.4f}.pth".format(epoch_label, loss_itm)
        save_filename = "%.3d_net_%.5f.pth"%(epoch_label, loss_itm)
    ...
    def update_learning_rate(self, metric=None, epoch=None):
        ...
        print('current learning rate = %.10f' % lr)
