...
from models.networks.ASPP_model import ASPP

class unet_CT_att_dsv_2D(nn.Module):
    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3,
        ...
        super(unet_CT_att_dsv_2D, self).__init__()
...
class unet_CT_att_2D(nn.Module):
    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3,
                 nonlocal_mode='concatenation', attention_dsample=(2,2), is_batchnorm=True):
        super(unet_CT_att_dsv_2D, self).__init__()
        self.is_deconv = is_deconv
...
class unet_CT_2D_ASFF(nn.Module):
    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3,
                 nonlocal_mode='concatenation', attention_dsample=(2,2), is_batchnorm=True):
        super(unet_CT_2D_ASFF, self).__init__()
        self.is_deconv = is_deconv
        self.in_channels = in_channels
        self.is_batchnorm = is_batchnorm
        self.feature_scale = feature_scale
        ...
        # final conv (without any concat)
        self.final = nn.Conv2d(n_classes*4, n_classes, 1)
        self.final_2 = nn.Conv2d(32, n_classes, 1)
    ...
    def forward(self, inputs):
        ...
        #dsv3 = self.dsv3(up3)
        #dsv2 = self.dsv2(up2)
        A_2 = self.ASFF_2(up3,up2,up1)
        out = self.final_2(A_2)
        #dsv1 = self.dsv1(up1)
        #final = self.final(torch.cat([dsv1,dsv2,dsv3,dsv4], dim=1))
...
class unet_CT_2D_ASPP(nn.Module):
    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3,
                 nonlocal_mode='concatenation', attention_dsample=(2,2), is_batchnorm=True):
        super(unet_CT_2D_ASPP, self).__init__()
        self.is_deconv = is_deconv
        self.in_channels = in_channels
        self.is_batchnorm = is_batchnorm
        self.feature_scale = feature_scale
        ...
        self.ASPP=ASPP(dim_in=filters[3],dim_out=filters[4])
...
class unet_CT_two_seg(nn.Module):
    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3,
                 nonlocal_mode='concatenation', attention_dsample=(2,2), is_batchnorm=True):
        super(unet_CT_two_seg, self).__init__()
        self.is_deconv = is_deconv
        self.in_channels = in_channels
        self.is_batchnorm = is_batchnorm
        self.feature_scale = feature_scale
        ...
        # deep supervision
        self.dsv4_2 = UnetDsv2(in_size=filters[3], out_size=n_classes, scale_factor=8)
        self.dsv3_2 = UnetDsv2(in_size=filters[2], out_size=n_classes, scale_factor=4)
        self.dsv2_2 = UnetDsv2(in_size=filters[1], out_size=n_classes, scale_factor=2)
        self.dsv1_2 = nn.Conv2d(in_channels=filters[0], out_channels=n_classes, kernel_size=1)
        
        # final conv (without any concat)
        #self.final_1 = nn.Conv2d(n_classes*4, n_classes, 1)
    ...
    def forward(self, inputs):
        ...
        # Deep Supervision
        dsv4 = self.dsv4_2(up4)
        dsv3 = self.dsv3_2(up3)
        dsv2 = self.dsv2_2(up2)
        dsv1 = self.dsv1_2(up1)
        #final = self.final(torch.cat([dsv1,dsv2,dsv3,dsv4], dim=1))
        filver = self.final_1(torch.cat([dsv1,dsv2,dsv3,dsv4], dim=1))
        cancer = self.final_2(torch.cat([dsv1,dsv2,dsv3,dsv4], dim=1))
        filver = self.apply_argmax_softmax(filver)
        cancer = self.apply_argmax_softmax(cancer)
        return [filver,cancer]
...
if __name__=='__main__':
    ...
    model=unet_CT_att_dsv_2D(feature_scale=4,n_classes=2,in_channels=1)
    sip=model(x)
